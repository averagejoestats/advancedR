...
#fname <- "CRNH0203-2023-NY_Ithaca_13_E.txt"
#url <- paste0("https://www.ncei.noaa.gov/pub/data/uscrn/products/hourly02/2023/",fname)
...
dat$date <- as.Date( as.character(dat$V2), format = "%Y%m%d" )
dat$year <- as.numeric( format(dat$date, "%Y"))
dat$doy  <- as.numeric( format(dat$date, "%j"))
dat$dom  <- as.numeric( format(dat$date, "%d"))
dat$dom_decimal <- dat$dom + dat$V3/2400
dat$doy_decimal <- dat$doy + dat$V3/2400
...
mtext("February Temperatures Ithaca 2004-2023",line = 1 )
